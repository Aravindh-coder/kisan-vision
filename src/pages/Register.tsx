...
export default function Register() {
  ...
  const inp: React.CSSProperties = { width: '100%', background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(74,222,128,0.2)', borderRadius: '12px', padding: '14px 16px', color: '#f0fdf4', fontSize: '14px', outline: 'none', marginBottom: '12px', boxSizing: 'border-box' }
    ...
    <div style={{ minHeight: '100vh', background: '#030a03', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'system-ui, Arial, sans-serif', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, background: 'rgba(2,10,2,0.95)', border: '1px solid rgba(74,222,128,0.3)', borderRadius: '20px', padding: '40px', width: '100%', maxWidth: '420px', backdropFilter: 'blur(20px)', boxShadow: '0 20px 60px rgba(0,0,0,0.8), 0 0 40px rgba(74,222,128,0.05)' }}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <p style={{ color: '#365f45', fontSize: '14px', margin: 0 }}>{t.createAccount}</p>
        </div>
        ...
        {success && <div style={{ background: 'rgba(22,163,74,0.15)', border: '1px solid rgba(74,222,128,0.3)', borderRadius: '10px', padding: '10px 14px', color: '#4ade80', marginBottom: '16px', fontSize: '13px' }}>✅ {success}</div>}
        <form onSubmit={handleSubmit}>
          <input type="text" placeholder={t.namePlaceholder} value={form.name} onChange={e => setForm({...form, name: e.target.value})} style={inp} />
          <input type="email" placeholder={t.emailPlaceholder} value={form.email} onChange={e => setForm({...form, email: e.target.value})} style={inp} />
          <input type="password" placeholder={t.passwordPlaceholder} value={form.password} onChange={e => setForm({...form, password: e.target.value})} style={{ ...inp, marginBottom: '20px' }} />
        ...
        <p style={{ color: '#365f45', marginTop: '20px', textAlign: 'center', fontSize: '13px' }}>
          {t.haveAccount} <Link to="/login" style={{ color: '#4ade80', textDecoration: 'none', fontWeight: 700 }}>{t.loginLink}</Link>
        </p>
      </div>
    </div>
  )
